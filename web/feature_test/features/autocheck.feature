Feature: As a Student I should be able to upload my deliveries and trigger the automatic correction
    ...
    Scenario: As a teacher browse the results of an automatic check
    	Given Teacher "teacher" exists with password "teacher"
          ...
          And autocheck for all deliveries has status "successfull"
          And autocheck for all deliveries has stdout "this is the successfull bash script"
          And autocheck for all deliveries has exit_value "0"
          And I log in as "teacher" "teacher"
