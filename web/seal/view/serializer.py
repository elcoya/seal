...
class RichAutomaticCorrectionList(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        pending_automatic_corrections = AutomaticCorrection.objects.filter(status=0)
        automatic_corrections = []
        for automatic_correction in pending_automatic_corrections:
            if automatic_correction.delivery.practice.get_script() is not None:
                automatic_corrections.append(automatic_correction)
        return automatic_corrections
